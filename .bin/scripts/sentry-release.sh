...
export SENTRY_DSN="$(sops --decrypt --extract '["LBA_SERVER_SENTRY_DSN"]' .infra/env.global.yml)"
cd "$ROOT_DIR/server"
"./sentry-release-server.sh" "mission-apprentissage/labonnealternance" "${COMMIT_ID}" "${PREV_COMMIT_ID}" "${VERSION}"

export SENTRY_DSN="$(sops --decrypt --extract '["LBA_UI_SENTRY_DSN"]' .infra/env.global.yml)"
cd "$ROOT_DIR/ui"
