...
- **`react-notion-x` est patché** (`.yarn/patches/`, RGAA 8.9 : blocs texte en `<p>`), et `resolutions` le fige sur 7.10.0 : modifier `ui/package.json` ne change pas la version installée. Pour monter de version, `yarn patch react-notion-x@npm:<version>`, reporter le changement, `yarn patch-commit -s <dossier>`, mettre à jour la clé de `resolutions`, puis lancer `notion-renderer-patch.test.ts`.
