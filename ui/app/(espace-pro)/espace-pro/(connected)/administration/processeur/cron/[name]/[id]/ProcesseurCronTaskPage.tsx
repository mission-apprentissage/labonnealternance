...
export default function JobInstancePage({ params }: { params: Promise<{ name: string; id: string }> }) {
  ...
  return (
    <Box>
      <Breadcrumb pages={[PAGES.static.adminProcessor, PAGES.dynamic.adminProcessorCron(name), PAGES.dynamic.adminProcessorCronTask({ name, id })]} />
      <Typography variant="h2" gutterBottom>
        {PAGES.dynamic.adminProcessorCronTask({ name, id }).title}
      </Typography>
      <ProcessorStatusProvider>
        {(status) => (
          <ProcessorStatusTaskComponent name={name} status={status} baseUrl={new URL(PAGES.static.adminProcessor.getPath(), publicConfig.baseUrl).href} id={id} type="cron" />
        )}
      </ProcessorStatusProvider>
    </Box>
  )
