...
export const ApplicationFormikSchema = ZApplicationApiPrivate.pick({
  ...
  applicant_first_name: z.string({ error: "Le prénom est obligatoire" }).trim().min(1, "Le prénom est obligatoire"),
  applicant_last_name: z.string({ error: "Le nom est obligatoire" }).trim().min(1, "Le nom est obligatoire"),
