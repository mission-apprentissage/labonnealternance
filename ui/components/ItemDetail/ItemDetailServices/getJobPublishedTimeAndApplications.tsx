...
export default function getJobPublishedTimeAndApplications({ item }: { item: ILbaItemJobsGlobal }) {
  ...
  const res = (
    <Box sx={{ display: "flex", textAlign: "center" }}>
      {hasJob(item) && item.job.creationDate && <Box sx={{ color: "grey.600", fontSize: "12px", mr: fr.spacing("8v") }}>Publiée depuis {getDaysSinceDate(item.job.creationDate)} jour(s)</Box>}
      {(item.ideaType === LBA_ITEM_TYPE.OFFRES_EMPLOI_LBA || item.ideaType === LBA_ITEM_TYPE.RECRUTEURS_LBA) && (
        <Box sx={{ display: "flex", textAlign: "center" }}>
          ...
          <Box sx={{ color: "#0063CB", fontSize: "12px", whiteSpace: "nowrap", ml: fr.spacing("2v") }}>{item.applicationCount} candidature(s)</Box>
        </Box>
      )}
    </Box>
  )
