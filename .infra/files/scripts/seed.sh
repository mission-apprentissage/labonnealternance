...
shift
# MongoDB partagé du stack preview-system (une base preview_<PR> par preview).
readonly MONGODB_CONTAINER="lba_mongodb"
...
docker exec "$MONGODB_CONTAINER" mongosh "mongodb://__system:{{ MONGODB_KEYFILE }}@localhost:27017/$TARGET_DB?authSource=local&directConnection=true" --eval "db.dropDatabase()" || true

# Restore from file copied into the container
echo "Restoring database $TARGET_DB..."
docker exec --user root "$MONGODB_CONTAINER" mongorestore \
