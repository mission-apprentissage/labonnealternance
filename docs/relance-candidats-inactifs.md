...
- **CTA personnalisé** : si `application_url` donne une recherche exploitable (un métier `q`, ou à défaut un lieu géolocalisé), `LIEN_RECHERCHE` contient un lien `/recherche?…` rejouant la propre recherche du candidat (métier + lieu exacts).
- **CTA générique** : sinon (`application_url` absent — candidatures API/widget/partenaires — ou fiche ouverte hors moteur, donc sans `?from=` ni paramètre legacy), `LIEN_RECHERCHE` est **vide**.
...
Unitaires sur la construction d'URL (lecture du `?from=`, traduction des URL legacy, retour à la 1ʳᵉ page, gestion des `utm_*`, retour `null` si rien d'exploitable) + intégration (MongoDB réelle, Brevo/Slack mockés) : push de la fenêtre J+7, exclusion des hors-fenêtre et des déjà-relancés, `LIEN_RECHERCHE` vide en fallback.
