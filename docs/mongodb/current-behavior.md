...
- **Preview — mongot désauthentifié (« Topology cache is unavailable », commandes `Unauthorized`)** : le `createUser` de [`preview.yml`](../../.infra/ansible/preview.yml) lit **le même fichier** que mongot, mais **l'archive de seed contient `admin.system.users`** : chaque `mongorestore` écrase `mongotUser` avec le mot de passe d'un autre environnement (signature : `mechanisms` SCRAM-SHA-1+256 au lieu du SCRAM-SHA-256 seul). D'où la tâche « Resynchroniser mongotUser après seed » de [`preview_pr.yml`](../../.infra/ansible/tasks/preview_pr.yml) — ne pas la retirer. Symptômes : `$listSearchIndexes`/création d'index en échec, recherches vides, logs mongot `Command getCmdLineOpts requires authentication`. Correctif : relancer un deploy preview (le resync est rejoué), ou à la main sur le serveur :
  ```bash
