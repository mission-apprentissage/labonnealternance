...
export const createEtablissementDelegation = ({ data, offreId }) =>
  API.post(`/formulaire/offre/${offreId}/delegation`, data)
...
export const deleteEntreprise = async (userId, formId) =>
  await API.delete(`/user`, { params: { userId, formId } }).catch(errorHandler)

// Temporaire, en attendant d'ajuster le modèle pour n'avoir qu'une seul source de données pour les entreprises
export const updateEntreprise = async (userId, formId, values) =>
  await Promise.all([updateUser(userId, values), putFormulaire(formId, values)])
