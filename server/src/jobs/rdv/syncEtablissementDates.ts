...
export const syncEtablissementDates = async () => {
  const etablissementByGestionnaireSiret: IEtablissementByGestionnaireSiret = await Etablissement.aggregate([
    {
      $group: {
  ...
  if (etablissementByGestionnaireSiret.length) {
    for await (const etablissement of etablissementByGestionnaireSiret) {
      ...
      if (!isEmpty(earliestDates)) {
        await Etablissement.updateMany({ gestionnaire_siret: etablissement._id }, { $set: earliestDates })
      }
    }
  }
