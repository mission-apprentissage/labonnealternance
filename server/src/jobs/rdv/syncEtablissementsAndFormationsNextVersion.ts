...
const updateLieuFormationEmail = async () => {
  const etfa: IEligibleTrainingsForAppointment[] = await db.collection("testETFA").find({}).project({ etablissement_gestionnaire_siret: 1, lieu_formation_email: 1 }).toArray()
  await Promise.all(
    etfa.map(async (formation) => {
      ...
      if (!formation.lieu_formation_email) {
        ...
        if (!email) return
        await db.collection("testETFA").findOneAndUpdate({ _id: formation._id }, { $set: { lieu_formation_email: email } })
        return
      }
...
const addReferrersToETFA = async () => {
  ...
  await Promise.all(
    etfa.map(async (formation) => {
      ...
      await db.collection("testETFA").findOneAndUpdate({ _id: formation._id }, { $set: { referrers: referrersToActivate } })
    })
  )
}
...
const updateGestionnaireEmailEtablissement = async () => {
  ...
  await asyncForEach(etablissements, async (etab) => {
    ...
    await Etablissement.findByIdAndUpdate(etab._id, { $set: { gestionnaire_email: email } })
  })
}
