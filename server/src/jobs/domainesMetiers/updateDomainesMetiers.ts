    ...
    for (let i = 0; i < onglet.length; i++) {
      ...
      if (metier) {
        ...
        const paramsDomaineMetier: IDomainesMetiers = {
          ...
          mots_clefs_sans_accent: removeAccents([...new Set(motsClefsDomaine)].join(", ")),
          appellations_romes: [...new Set(appellationsROMEs)].join(", "),
          appellations_romes_sans_accent: removeAccents([...new Set(appellationsROMEs)].join(", ")),
          couples_appellations_rome_metier: coupleAppellationsRomeIntitules,
          codes_romes: codesROMEs,
          intitules_romes: intitulesROMEs,
          intitules_romes_sans_accent: intitulesROMEs.map((v) => removeAccents(v)),
          codes_rncps: codesRNCPs,
          intitules_rncps: intitulesRNCPs,
          intitules_rncps_sans_accent: intitulesRNCPs.map((v) => removeAccents(v)),
          couples_romes_metiers: couplesROMEsIntitules,
          codes_fap: [...new Set(codesFAPs)],
          intitules_fap: [...new Set(libellesFAPs)],
          intitules_fap_sans_accent: [...new Set(libellesFAPs)].map((v) => removeAccents(v)),
          sous_domaine_onisep: sousDomainesOnisep,
          sous_domaine_onisep_sans_accent: sousDomainesOnisep.map((v) => removeAccents(v)),
          created_at: new Date(),
