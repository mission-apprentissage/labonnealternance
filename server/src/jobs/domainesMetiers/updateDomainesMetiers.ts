    ...
    for (let i = 0; i < onglet.length; i++) {
      ...
      if (metier) {
        ...
        const paramsDomaineMetier: IDomainesMetiers = {
          ...
          mots_clefs_specifiques_sans_accent_computed: removeAccents([...new Set(motsClefsSpecifiques)].join(", ")),
          mots_clefs: [...new Set(motsClefsDomaine)].join(", "),
          mots_clefs_sans_accent_computed: removeAccents([...new Set(motsClefsDomaine)].join(", ")),
          appellations_romes: [...new Set(appellationsROMEs)].join(", "),
          appellations_romes_sans_accent_computed: removeAccents([...new Set(appellationsROMEs)].join(", ")),
          ...
          intitules_fap_sans_accent_computed: [...new Set(libellesFAPs)].map((v) => removeAccents(v)),
          sous_domaine_onisep: sousDomainesOnisep,
          sous_domaine_onisep_sans_accent_computed: sousDomainesOnisep.map((v) => removeAccents(v)),
          created_at: new Date(),
