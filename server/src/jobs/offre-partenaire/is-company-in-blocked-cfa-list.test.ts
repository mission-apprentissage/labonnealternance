...
import { getCompanyInBlockedCfaList, isCompanyInBlockedCfaList } from "./is-company-in-blocked-cfa-list"
...
describe("isCompanyInBlockedCfaList (insensible à la casse et aux accents)", () => {
  ...
  it("devrait trouver les nouvelles entrées ajoutées à la liste des CFA bloqués", () => {
    expect(isCompanyInBlockedCfaList("EEMI")).toBe(true)
    expect(isCompanyInBlockedCfaList("ESIC")).toBe(true)
    expect(isCompanyInBlockedCfaList("FORMATION DIGITAL SKILLS")).toBe(true)
    expect(isCompanyInBlockedCfaList("IEF2I EDUCATION")).toBe(true)
    expect(isCompanyInBlockedCfaList("IFCE")).toBe(true)
    expect(isCompanyInBlockedCfaList("IFCE INSTITUT FORMATION COMPTABLE EUROPE")).toBe(true)
  })
  ...
  it("ne doit pas matcher le sigle court IFCE lorsqu'il est inclus dans un autre mot", () => {
    expect(isCompanyInBlockedCfaList("Office du tourisme")).toBe(false)
  })
})
