...
import { jobsPartnersToApiV3Read } from "@/services/jobs/job-opportunity/job-opportunity.service"
...
export async function exportJobsToS3V2(handleFileReadStream = uploadFileToS3) {
  ...
  const recruiterTransform = getCusorTransform<IJobsPartnersOfferPrivate>((jobPartner) => {
    return JSON.stringify(jobsPartnersToApiV3Read(jobPartner), null, 2)
  })
  ...
  const lbaCursor = await getDbCollection("jobs_partners").find({
    partner_label: { $eq: JOBPARTNERS_LABEL.OFFRES_EMPLOI_LBA },
    offer_status: JOB_STATUS_ENGLISH.ACTIVE,
    offer_multicast: true,
  })
  await pipeline(lbaCursor, recruiterTransform)
