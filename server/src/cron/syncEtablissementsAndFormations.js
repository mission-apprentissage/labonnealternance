...
export const syncEtablissementsAndFormations = async ({ etablissements, widgetParameters }) => {
  ...
  do {
    const page = response?.pagination?.page ? Number(response?.pagination?.page) + 1 : 21;
      ...
      response.formations.map(async (formation) => {
        ...
        if (widgetParameter) {
          ...
          if (!widgetParameter?.is_custom_email_rdv) {
            emailRdv =
              getEmailFromCatalogueField(formation.etablissement_formateur_courriel) || widgetParameter.email_rdv;
          }
