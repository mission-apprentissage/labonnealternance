...
describe("jobEtFormationV1", () => {
  ...
  /*it("Vérifie que la recherche répond avec des résultats", async () => {
    ...
    assert.strictEqual(response.status, 200);
    assert.ok(response.body.formations.results instanceof Array);
    assert.ok(response.body.jobs.matchas.results instanceof Array);
    assert.strictEqual(response.body.jobs.peJobs.status, 401);
    assert.strictEqual(response.body.jobs.lbaCompanies.status, 401);
  });*/
  ...
  /*it("Vérifie que la recherche répond avec des résultats", async () => {
    ...
    assert.strictEqual(response.status, 200);
    assert.ok(response.body.formations === null);
    assert.ok(response.body.jobs.peJobs === null);
    //assert.ok(response.body.jobs.lbaCompanies.results instanceof Array);
    assert.strictEqual(response.body.jobs.lbaCompanies.status, 401);
    assert.ok(response.body.jobs.lbbCompanies === null);
  });*/
  ...
  it.skip("Vérifie que les requêtes sans ROME sont refusées", async () => {
    ...
    assert.ok(response.body.error_messages.indexOf("romes : Rome codes are missing. At least 1.") >= 0)
  })
  ...
  it.skip("Vérifie que les requêtes avec trop de ROME sont refusées", async () => {
    ...
    assert.strictEqual(response.status, 400)
    assert.deepStrictEqual(response.body.error, "wrong_parameters")
    assert.ok(response.body.error_messages.indexOf("romes : Too many rome codes. Maximum is 15.") >= 0)
  })
  ...
  it("Vérifie que les requêtes sans code insee sont refusées", async () => {
    ...
    assert.strictEqual(response.status, 400)
    assert.deepStrictEqual(response.body.error, "wrong_parameters")
    assert.ok(response.body.error_messages.indexOf("insee : insee city code is missing.") >= 0)
  })
  ...
  it("Vérifie que les requêtes avec code insee mal formé sont refusées", async () => {
    ...
    assert.strictEqual(response.status, 400)
    assert.deepStrictEqual(response.body.error, "wrong_parameters")
    assert.ok(response.body.error_messages.indexOf("insee : Badly formatted insee city code. Must be 5 digit number.") >= 0)
  })
