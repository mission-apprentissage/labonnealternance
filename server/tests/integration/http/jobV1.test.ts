...
describe.skip("jobV1", () => {
  ...
  it("Vérifie que la recherche répond avec des résultats", async () => {
    ...
    assert.strictEqual(response.status, 200)
    /*assert.ok(response.body.peJobs.results instanceof Array);
    assert.ok(response.body.lbaCompanies.results instanceof Array);
    assert.ok(response.body.lbbCompanies.results instanceof Array);*/
    assert.strictEqual(response.body.peJobs.status, 401)
    //assert.ok(response.body.lbaCompanies.results instanceof Array);
    assert.ok(response.body.matchas.results instanceof Array)
  })
