...
describe("create-many-applications", () => {
  it("create-many-applications", () => {
    ...
    givenAMatchaOffer((recruiter) => {
      ...
      cy.request({
        method: "GET",
        url: `${Cypress.env("server")}/api/v1/jobs/matcha/${jobId}`,
      }).then((response) => {
        const { token } = response.body.matchas[0]

        for (let i = 0; i < 100; i++) {
          cy.request({
            method: "POST",
            url: Cypress.env("server") + "/api/v2/_private/application",
            headers: {
